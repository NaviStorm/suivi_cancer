...
class _CycleDetailsContent extends StatelessWidget {
          ...
          provider.isLoading
              ...
              : CustomScrollView(
                slivers: [
                  ...
                  SliverToBoxAdapter(
                    child: CycleInfoCard(cycle: provider.cycle!),
                  ),
                  SliverPadding(
  ...
  void _showAddEventActionSheet(BuildContext context) {
    showCupertinoModalPopup(
      context: context,
      builder:
          (modalContext) => CupertinoActionSheet(
            title: const Text('Ajouter un événement'),
            actions: <CupertinoActionSheetAction>[
              CupertinoActionSheetAction(
                child: const Text('Séance de traitement'),
                onPressed: () {
                  Navigator.pop(modalContext);
                  _navigateToAddSession(context);
                },
              ),
              CupertinoActionSheetAction(
                child: const Text('Examen médical'),
                onPressed: () {
                  Navigator.pop(modalContext);
                  _navigateToAddExamination(context);
                },
              ),
              CupertinoActionSheetAction(
                ...
                onPressed: () {
                  Navigator.pop(modalContext);
                  _navigateToAddAppointment(context);
                },
              ),
              CupertinoActionSheetAction(
                child: const Text('Prise de médicament'),
                onPressed: () {
                  Navigator.pop(modalContext);
                  _navigateToAddMedicationIntake(context);
                },
              ),
              CupertinoActionSheetAction(
          ...
          (modalContext) => CupertinoActionSheet(
            actions: <CupertinoActionSheetAction>[
              CupertinoActionSheetAction(
                ...
                onPressed: () {
                  Navigator.pop(modalContext);
                  _editMedicationIntake(context, intake);
                },
              ),
              CupertinoActionSheetAction(
                child: const Text('Dupliquer pour aujourd\'hui'),
                onPressed: () {
                  Navigator.pop(modalContext);
                  _duplicateMedicationIntake(context, intake);
                },
              ),
              CupertinoActionSheetAction(
                ...
                onPressed: () {
                  Navigator.pop(modalContext);
                  _confirmDeleteEvent(context, intake);
                },
              ),
            ],
            cancelButton: CupertinoActionSheetAction(
  ...
  Future<void> _toggleEventCompleted(
    BuildContext context,
    dynamic event,
  ) async {
    final provider = Provider.of<CycleProvider>(context, listen: false);
    try {
      if (event is Session)
        await provider.toggleSessionCompleted(event);
      else if (event is Examination)
        await provider.toggleExaminationCompleted(event);
      else if (event is MedicationIntake)
        await provider.toggleMedicationIntakeCompleted(event);
    } catch (e) {
      _showInfoDialog(
        context,
        "Erreur",
  ...
  void _navigateToEventDetails(
    ...
    provider.safeNavigate(() async {
      ...
      if (type == 'session') {
        ...
        // refresh = result == true;
        // Obligé de forcer le refresh même si rien car pour l'etablissement, le refresh ne fonctionne pas ()
        refresh = true;
    ...
    final examinationType = await showCupertinoModalPopup<String>(
      ...
      builder:
          (modalContext) => CupertinoActionSheet(
            ...
            actions: <CupertinoActionSheetAction>[
              CupertinoActionSheetAction(
                child: const Text('Pour le cycle entier'),
                onPressed: () => Navigator.pop(modalContext, 'cycle'),
              ),
              CupertinoActionSheetAction(
                child: const Text('Pour une séance spécifique'),
                onPressed: () => Navigator.pop(modalContext, 'session'),
              ),
              CupertinoActionSheetAction(
                child: const Text('Pour toutes les séances'),
                onPressed: () => Navigator.pop(modalContext, 'all_sessions'),
              ),
            ],
            cancelButton: CupertinoActionSheetAction(
              child: const Text('Annuler'),
          ...
          actions:
              provider.sessions.map((session) {
                ...
                return CupertinoActionSheetAction(
                  child: Text(
                    'Séance du $date ${session.isCompleted ? "(Terminée)" : ""}',
                  ),
  ...
  void _navigateToAddAppointment(BuildContext context) {
    ...
    provider.safeNavigate(() async {
      final result = await Navigator.push(
        ...
        CupertinoPageRoute(
          builder:
              (context) => AddAppointmentScreen(cycleId: provider.cycle!.id),
        ),
      );
      if (result == true) {
        provider.refreshCycleData();
  ...
  void _navigateToAddDocument(BuildContext context) {
    _showInfoDialog(
      context,
      'Indisponible',
      "L'ajout de documents sera bientôt disponible.",
    );
  }
  ...
  void _openDocument(BuildContext context, Document document) {
    _showInfoDialog(
      context,
      'Indisponible',
      "L'ouverture de documents sera bientôt disponible.",
    );
  }
  ...
  void _navigateToEditCycle(BuildContext context) {
    ...
    provider.safeNavigate(() async {
      _showInfoDialog(
        context,
        'Indisponible',
        "L'édition du cycle sera bientôt disponible.",
      );
    });
  }
  ...
  Future<void> _confirmCompleteCycle(BuildContext context) async {
    ...
    final confirmed = await _showConfirmationDialog(
      ...
      content: 'Voulez-vous vraiment marquer ce cycle comme terminé ?',
      isDestructive: false,
    );
    if (confirmed) {
      try {
