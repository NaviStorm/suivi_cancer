...
class _AddRadiotherapyScreenState extends State<AddRadiotherapyScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_isEditMode) {
        ...
        _selectedDoctors = radiotherapy.doctors;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditMode ? 'Modifier la radiothérapie' : 'Nouvelle radiothérapie'),
      ),
      body: _isLoading
          ...
          child: Column(
            ...
            children: [
              ...
              CustomTextField(
                ...
                maxLines: 4,
                placeholder: 'Ajoutez des notes importantes concernant cette radiothérapie',
              ),
              SizedBox(height: 32),
              ElevatedButton(
  ...
  Future<void> _saveRadiotherapy() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final dbHelper = DatabaseHelper();
        final String radiotherapyId = _isEditMode ? widget.radiotherapy!.id : Uuid().v4();
        ...
        Map<String, dynamic> radiotherapyData = {
          ...
          'isCompleted': _isEditMode ? (widget.radiotherapy!.isCompleted ? 1 : 0) : 0,
