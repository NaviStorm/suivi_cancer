...
class CycleProvider extends ChangeNotifier {
  ...
  Future<void> _loadExaminations() async {
    try {
      ...
      _examinations = examinationMaps.map((map) => Examination.fromMap(map)).toList();
    } catch (e) {
  ...
  Future<bool> toggleMedicationIntakeCompleted(MedicationIntake intake) async {
    try {
      ...
      Log.e("Erreur lors de la mise à jour de l'état de la prise de médicament: $e");
      throw e;
    }
  }
  ...
  List<Map<String, dynamic>> getAllEvents() {
    ...
    for (var session in _sessions) {
      ...
      allEvents.add(EventMapper.mapSessionToEvent(session, _getSessionNumberInCycle(session)));
    }
