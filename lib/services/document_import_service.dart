...
class DocumentImportService {
        ...
        switch (fileExtension) {
          case 'pdf':
            ...
            if (await isPdfPasswordProtected(filePath)) {
              return await handlePasswordProtectedPdf(
                context,
                filePath,
                fileName,
              );
  ...
  Future<Document?> processDocument(
    ...
    try {
      ...
      final docName = await getDocumentName(
        context,
        docType.toString(),
        fileName,
      );
      ...
      final absolutePath =
          '${await getApplicationDocumentsDirectory().then((dir) => dir.path)}/$relativePath';
  ...
  Future<Document?> handlePasswordProtectedPdf(
    ...
    final choice = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              'Ce document PDF est protégé par un mot de passe. Souhaitez-vous supprimer cette protection ?',
            ),
            actions: [
              TextButton(
    ...
    if (choice == true) {
      ...
      if (password != null && password.isNotEmpty) {
        final decryptedFilePath = await decryptPdf(
          context,
          filePath,
          fileName,
          password,
  ...
  Future<String?> decryptPdf(
    ...
    try {
      ...
      try {
        ...
        _showMessage(
          context,
          'Protection par mot de passe supprimée avec succès',
        );
        ...
        _showErrorMessage(
          context,
          "Mot de passe incorrect ou document non déchiffrable",
        );
        return null;
      }
    } catch (e) {
            ...
            (context) => Scaffold(
              ...
              body: Center(
                child: InteractiveViewer(
                  minScale: 0.5,
                  maxScale: 4.0,
                  child: Image.file(File(absolutePath), fit: BoxFit.contain),
                ),
              ),
            ),
      ),
                ...
                (context) => Scaffold(
                  ...
                  body: PDFView(
                    ...
                    onPageError: (page, error) {
                      _showErrorMessage(
                        context,
                        "Erreur lors du chargement de la page $page: $error",
                      );
                    },
                  ),
                ),
  ...
  Future<void> openWithDefaultApp(
    BuildContext context,
    Document document,
  ) async {
    try {
